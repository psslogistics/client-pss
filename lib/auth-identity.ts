...
export const AUTH_IDENTITY_KEY = "pss_client_auth_identity_v1";
...
export function deriveAuthIdentity(identifier: string): AuthIdentity | null {
  ...
  if (emailMatch) {
    ...
    return { identifier: value, username: emailMatch[1].toLowerCase(), method: "email", email, signedInAt: new Date().toISOString() };
  }
